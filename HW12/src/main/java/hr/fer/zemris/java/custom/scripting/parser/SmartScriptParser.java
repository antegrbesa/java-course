...
/**
 * Implementation of a parser that makes use of a {@link SmartScriptLexer}.
 * Parses text with specifications given in {@link SmartScriptLexer} class.
...
public class SmartScriptParser {
	...
	private void parse() {
		...
		while((token = lexer.nextToken()).getType() != SmartScriptTokenType.EOF) {
			...
			if(type == SmartScriptTokenType.START_TAG) {	//process the whole tag
				token = lexer.nextToken();
				type = token.getType();
				
				if(type == SmartScriptTokenType.ECHO) {	//tag is an echo tag
					...
					while((token = lexer.nextToken()).getType() != SmartScriptTokenType.END_TAG) {
				...
				if(type == SmartScriptTokenType.VARIABLE) {	//tag is a for loop
	 ...
	 */
	private Element getElement(SmartScriptToken token, boolean echo) {
		if(token == null) {
		...
		if(type == SmartScriptTokenType.FUNCTION && echo) {
		...
		if(type == SmartScriptTokenType.OPERATOR && echo) {
			return new ElementOperator((String) token.getValue());
		}
		if(type == SmartScriptTokenType.CONSTANT_INTEGER) {
			return new ElementConstantInteger((int) token.getValue());
		}
		if(type == SmartScriptTokenType.CONSTANT_DOUBLE) {
